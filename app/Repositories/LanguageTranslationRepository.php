...
class LanguageTranslationRepository {
	...
	public function getAlldata($arraydata){
		...
		if(CommonTools::checkArrayValue($arraydata,'lt_taiwan')){
			if($arraydata['search_type']){
				$string->where(function ($query) use ($arraydata) {
					$query->where('cm_languagetranslation.lt_taiwan', $arraydata['lt_taiwan'])
						->orWhere('cm_languagetranslation.lt_china', $arraydata['lt_taiwan'])
						->orWhere('cm_languagetranslation.lt_english',$arraydata['lt_taiwan'])
						->orWhere('cm_languagetranslation.lt_japan',$arraydata['lt_taiwan'])
						->orWhere('cm_languagetranslation.lt_korea',$arraydata['lt_taiwan'])
						->orWhere('cm_languagetranslation.lt_vietnam',$arraydata['lt_taiwan'])
						->orWhere('cm_languagetranslation.lt_thailand',$arraydata['lt_taiwan']);
				});
			}else{
				$string->where(function($query) use ($arraydata){
